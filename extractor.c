...
static const int _MONITORROM_DMG_SIZE = 0x100;
static const int _MONITORROM_CGB_SIZE = 0x900;


unsigned char monitorRomDMG[_MONITORROM_DMG_SIZE] = {
...
unsigned char monitorRomMGB[_MONITORROM_DMG_SIZE] = {
	0x31,0xFE,0xFF,0xAF,0x21,0xFF,0x9F,0x32,0xCB,0x7C,0x20,0xFB,0x21,0x26,0xFF,0x0E,
	0x11,0x3E,0x80,0x32,0xE2,0x0C,0x3E,0xF3,0xE2,0x32,0x3E,0x77,0x77,0x3E,0xFC,0xE0,
	0x47,0x11,0x04,0x01,0x21,0x10,0x80,0x1A,0xCD,0x95,0x00,0xCD,0x96,0x00,0x13,0x7B,
	0xFE,0x34,0x20,0xF3,0x11,0xD8,0x00,0x06,0x08,0x1A,0x13,0x22,0x23,0x05,0x20,0xF9,
...
unsigned char monitorRomCGB[_MONITORROM_CGB_SIZE] = {
	...
	0x22,0x23,0x22,0x23,0x13,0x0D,0x20,0xF7,0x11,0x80,0xFF,0xCD,0x8F,0x03,0x01,0xA8,
    ...
    // splitting CGB into multiple pieces that skips over the header area
    unsigned char monitorRomCGBA[256] = {
	0x31,0xFE,0xFF,0x3E,0x02,0xC3,0x7C,0x00,0xD3,0x00,0x98,0xA0,0x12,0xD3,0x00,0x80,
	0x00,0x40,0x1E,0x53,0xD0,0x00,0x1F,0x42,0x1C,0x00,0x14,0x2A,0x4D,0x19,0x8C,0x7E,
	0x00,0x7C,0x31,0x6E,0x4A,0x45,0x52,0x4A,0x00,0x00,0xFF,0x53,0x1F,0x7C,0xFF,0x03,
	0x1F,0x00,0xFF,0x1F,0xA7,0x00,0xEF,0x1B,0x1F,0x00,0xEF,0x1B,0x00,0x7C,0x00,0x00,
    ...
    unsigned char monitorRomCGBB[256*7] = {
        0x21,0x00,0x80,0xAF,0x22,0xCB,0x6C,0x28,0xFB,0xC9,0x2A,0x12,0x13,0x0D,0x20,0xFA,
	0xC9,0xE5,0x21,0x0F,0xFF,0xCB,0x86,0xCB,0x46,0x28,0xFC,0xE1,0xC9,0x11,0x00,0xFF,
	0x21,0x03,0xD0,0x0E,0x0F,0x3E,0x30,0x12,0x3E,0x20,0x12,0x1A,0x2F,0xA1,0xCB,0x37,
	0x47,0x3E,0x10,0x12,0x1A,0x2F,0xA1,0xB0,0x4F,0x7E,0xA9,0xE6,0xF0,0x47,0x2A,0xA9,
...
int main() {
    ...
    //monitorRomDMG[0xFD] = 0x11;
    
    FILE *f = fopen("monitorromDMG.rom", "wb");
    fwrite(monitorRomDMG, sizeof(char), sizeof(monitorRomDMG), f);
    fclose(f);
    
    FILE *f2 = fopen("monitorromMGB.rom", "wb");
    fwrite(monitorRomMGB, sizeof(char), sizeof(monitorRomMGB), f2);
    fclose(f2);
    
    FILE *f3 = fopen("monitorromCGB.rom", "wb");
    fwrite(monitorRomCGB, sizeof(char), sizeof(monitorRomCGB), f3);
    fclose(f3);
    
    FILE *f4 = fopen("monitorromCGB-A.rom", "wb");
    fwrite(monitorRomCGBA, sizeof(char), sizeof(monitorRomCGBA), f4);
    fclose(f4);
    
    FILE *f5 = fopen("monitorromCGB-B.rom", "wb");
    fwrite(monitorRomCGBB, sizeof(char), sizeof(monitorRomCGBB), f5);
    fclose(f5);
